% Experiments that comparing the performance of different algorithms under different dimensions
...
xlim([1 20]);
y=cell(1,length(ind));
y(1:2:end)=num2cell(300:400:4100);
set(gca,'xtick',ind,'xticklabel',y);
set(gca,'FontName','Times New Roman','Fontsize',11);
h=legend('SOMP','MFOCUSS','MMV-SPG','SNIHT','MMV-ADMM-L_{2,1}','MMV-ADMM-L_{2,0}','Location','northwest');
set(h,'FontSize',8);
xlabel('N');
ylabel('Time(s)');
%title('Time of different algorithms with different sample dimensions');

%successful percentage
epsilon = 1e-5;

SuccessPercentage1 = zeros(1,length(N_set));
SuccessPercentage2 = zeros(1,length(N_set));
SuccessPercentage3 = zeros(1,length(N_set));
SuccessPercentage4 = zeros(1,length(N_set));
SuccessPercentage5 = zeros(1,length(N_set));
SuccessPercentage6 = zeros(1,length(N_set));
...
%title('Percentage of successful recovery of different algorithms with different sample dimensions');
