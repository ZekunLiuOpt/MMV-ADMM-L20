...
xlim([1.2 8.8]);
y=cell(1,length(ind));
y(1:2:end)=num2cell(1.2:0.8:8.8);
...
xlabel('N/M');
ylabel('Time(s)');
%title('Time of different algorithms with different undersampling rate');

%successful percentage
epsilon = 1e-5;

SuccessPercentage1 = zeros(1,length(M_set));
SuccessPercentage2 = zeros(1,length(M_set));
SuccessPercentage3 = zeros(1,length(M_set));
SuccessPercentage4 = zeros(1,length(M_set));
SuccessPercentage5 = zeros(1,length(M_set));
SuccessPercentage6 = zeros(1,length(M_set));
...
set(gca,'xtick',ind,'xticklabel',y);
set(gca,'FontName','Times New Roman','Fontsize',11);
h=legend('SOMP','MFOCUSS','MMV-SPG','SNIHT','MMV-ADMM-L_{2,1}','MMV-ADMM-L_{2,0}','Location','NorthEast');
set(h,'FontSize',8);
xlabel('N/M');
