...
xlim([0.06 0.25]);
y=cell(1,length(ind));
y(1:2:end)=num2cell(0.06:0.02:0.25);
...
xlabel('K/N');
ylabel('Time(s)');
%title('Time of different algorithms with different sparsity');

%successful percentage
epsilon = 1e-5;  % When RMSE<epsilon, this trial is said to be successful

SuccessPercentage1 = zeros(1,length(K_set));
SuccessPercentage2 = zeros(1,length(K_set));
SuccessPercentage3 = zeros(1,length(K_set));
SuccessPercentage4 = zeros(1,length(K_set));
SuccessPercentage5 = zeros(1,length(K_set));
SuccessPercentage6 = zeros(1,length(K_set));
...
set(gca,'xtick',ind,'xticklabel',y);
set(gca,'FontName','Times New Roman','Fontsize',11);
h=legend('SOMP','MFOCUSS','MMV-SPG','SNIHT','MMV-ADMM-L_{2,1}','MMV-ADMM-L_{2,0}','Location','southwest');
set(h,'FontSize',8);
xlabel('K/N');
